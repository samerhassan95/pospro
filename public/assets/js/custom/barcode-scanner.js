...
(function ($) {
    ...
    function initBarcodeScanner() {
        ...
        // Set initial class
        $(".products-section").removeClass(
            "view-active-category view-active-brand view-active-search view-active-tables",
        );
        ...
        scanBtn.off("click.barcode").on("click.barcode", function (e) {
            ...
            console.log(
                "🔍 DIRECT: Before - Search view display:",
                $("#search-view").css("display"),
            );
            console.log(
                "🔍 DIRECT: Before - Category view display:",
                $("#category-view").css("display"),
            );
            ...
            console.log(
                "🔍 DIRECT: After - Search view display:",
                $("#search-view").css("display"),
            );
            console.log(
                "🔍 DIRECT: After - Category view display:",
                $("#category-view").css("display"),
            );
            ...
            if (searchViewElement) {
                console.log(
                    "🔍 DIRECT: Search view element style:",
                    searchViewElement.style.display,
                );
                console.log(
                    "🔍 DIRECT: Search view computed style:",
                    window.getComputedStyle(searchViewElement).display,
                );
            }

            // Update button states
            $(".pos-view-btn, .pos-toggle-btn").removeClass(
                "pos-toggle-btn-active active",
            );
        ...
        $("#clear-search-btn, #clear-product-search-btn").on(
            "click",
            function () {
        ...
        $(".pos-view-btn").on("click", function () {
            ...
            if (view !== "search" && this.id !== "scan-barcode-btn") {
                ...
                console.log(
                    "🔄 After switch - Category view:",
                    $("#category-view").css("display"),
                );
                console.log(
                    "🔄 After switch - Search view:",
                    $("#search-view").css("display"),
                );
            }
        });
    ...
    function searchProducts(query, autoAdd = false) {
        ...
        $.ajax({
            url:
                $("#barcode-search-route").val() || "/business/products/search",
            method: "POST",
            data: {
                query: query,
            ...
            success: function (response) {
                ...
                if (
                    ...
                    if (autoAdd && response.products.length === 1) {
                        ...
                        const isPurchaseMode = $("#purchase-cart").length > 0;
                        const productData = {
                            id: product.id,
                            name: product.productName || product.name,
                            code: product.productCode || product.code || "",
                            price: isPurchaseMode
                                ? product.purchase_price || product.price || 0
                                : product.sales_price || product.price || 0,
                            image:
                                product.image ||
                        ...
                        if (typeof toastr !== "undefined") {
                            toastr.success(
                                "Product scanned and added to cart!",
                            );
                        }
                    }
                } else {
    ...
    function addProductToCart(product, $btn) {
        const cartType = $("#purchase-cart").length > 0 ? "purchase" : "sale";
        const cartData = {
            type: cartType,
            id: product.id,
            name: product.name,
    ...
    function startCameraScanning() {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            alert(
                "Camera access is not supported in this browser. Please use manual input.",
            );
            return;
        }
    ...
    function formatCurrency(amount) {
        ...
        // Fallback formatting with SAR SVG support
    ...
    function initGlobalBarcodeListener() {
        $(document).on("keypress", function (e) {
            ...
            if (
                ...
                !$target.is("#product-search-input")
            ) {
                return;
            }
            ...
            if (e.which === 13) {
                ...
                if (barcodeBuffer.length >= 3) {
                    // Minimum 3 characters for a barcode
                    e.preventDefault();
    ...
    $(document).ready(function () {
        ...
        setTimeout(function () {
            ...
            $(document)
                .off("click", "#scan-barcode-btn")
                .on("click", "#scan-barcode-btn", function (e) {
                    e.preventDefault();
            ...
            $(document)
                .off("click", ".pos-view-btn:not(#scan-barcode-btn)")
                .on(
                    "click",
                    ".pos-view-btn:not(#scan-barcode-btn)",
                    function (e) {
                        const view = $(this).data("view");
                        ...
                        if (view && view !== "search") {
                            ...
                            if (targetView.length) {
                                ...
                                console.log(
                                    "🔄 OVERRIDE: Showing view:",
                                    view + "-view",
                                );

                                // Add corresponding class to products section
                                $(".products-section").addClass(
                                    "view-active-" + view,
                                );
                            }
                            ...
                            console.log(
                                "🔄 OVERRIDE: View switch complete for:",
                                view,
                            );
                            return false;
                        }
