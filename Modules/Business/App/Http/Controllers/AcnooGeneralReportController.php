...
class AcnooGeneralReportController extends Controller
{
    ...
    public function lossProfit(Request $request)
    {
        ...
        $branchId = null;
        $hasBranchInSales = false;
        $hasBranchInIncomes = false;
        $hasBranchInExpenses = false;
        $hasBranchInPayrolls = false;
        ...
        if (moduleCheck('MultiBranchAddon')) {
            ...
            $hasBranchInSales = \Schema::hasColumn('sales', 'branch_id');
            $hasBranchInIncomes = \Schema::hasColumn('incomes', 'branch_id');
            $hasBranchInExpenses = \Schema::hasColumn('expenses', 'branch_id');
            $hasBranchInPayrolls = \Schema::hasColumn('payrolls', 'branch_id');
        }
        ...
        $salesQuery = DB::table('sales')
            ...
            ->when($branchId && $hasBranchInSales, fn($q) => $q->where('branch_id', $branchId))
        ...
        $incomeQuery = DB::table('incomes')
            ...
            ->where('business_id', $businessId)
            ->when($branchId && $hasBranchInIncomes, fn($q) => $q->where('branch_id', $branchId))
        ...
        if (moduleCheck('HrmAddon')) {
            $payrollQuery = DB::table('payrolls')
                ...
                ->where('business_id', $businessId)
                ->when($branchId && $hasBranchInPayrolls, fn($q) => $q->where('branch_id', $branchId))
        ...
        $expenseQuery = DB::table('expenses')
            ...
            ->where('business_id', $businessId)
            ->when($branchId && $hasBranchInExpenses, fn($q) => $q->where('branch_id', $branchId))
