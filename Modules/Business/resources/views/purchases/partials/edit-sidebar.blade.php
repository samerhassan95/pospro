...
<div class="order-sidebar">
    ...
    <div class="top-row-container">
        ...
        <div class="request-number-section">
            <div class="request-number-row">
                ...
                <span class="request-value" id="request-number-display">{{ $purchase->invoiceNumber }}</span>
            </div>
        </div>
        ...
        <div class="search-customer-wrapper">
            <select required name="party_id" id="party_id" class="sidebar-customer-select choices-select">
                ...
                @foreach ($suppliers as $supplier)
                    <option value="{{ $supplier->id }}" @selected($purchase->party_id == $supplier->id) data-type="{{ $supplier->type }}" data-phone="{{ $supplier->phone }}">
    ...
    <div class="order-details-section">
        <div class="customer-info-flex">
            <div class="customer-name" id="selected-customer-name">
                @if($purchase->party)
                    {{ $purchase->party->name }}
                @else
                    {{ __('Select Supplier') }}
            ...
            <div class="customer-phone" id="selected-customer-phone">
                @if($purchase->party && $purchase->party->phone)
                    {{ $purchase->party->phone }}
                @endif
            </div>
        </div>
        <div class="order-info-line">
            <span class="order-date">{{ formatted_date($purchase->purchaseDate, 'D, M\TH\LY Y') }}</span>
            <div class="time-with-icon">
                ...
                <span class="order-time" id="current-time">{{ formatted_date($purchase->purchaseDate, 'h:i A') }}</span>
            </div>
        </div>
        <input type="hidden" name="invoiceNumber" value="{{ $purchase->invoiceNumber }}">
        <input type="hidden" name="purchaseDate" value="{{ formatted_date($purchase->purchaseDate, 'Y-m-d') }}">
    </div>
    ...
    <div class="order-summary-section">
        ...
        <div class="summary-row discount-row">
            ...
            <div class="discount-controls">
                <span class="summary-value" id="discount_display">{!! currency_format($purchase->discountAmount ?? 0, currency: business_currency()) !!}</span>
        ...
        <div class="discount-input-section d-none" id="discount-input-section">
            <div class="discount-input-wrapper">
                <input type="number" step="0.01" min="0" id="discount_amount_input" class="form-control discount-input" placeholder="0.00" value="{{ ($purchase->discount_type ?? 'flat') == 'percent' ? ($purchase->discount_percent ?? 0) : ($purchase->discountAmount ?? 0) }}">
                <select id="discount_type_select" class="form-select discount-type-select">
                    <option value="flat" @selected(($purchase->discount_type ?? 'flat') == 'flat')>{{ __('Flat') }} ({{ business_currency()->symbol }})</option>
                    <option value="percent" @selected(($purchase->discount_type ?? 'flat') == 'percent')>{{ __('Percent') }}</option>
                </select>
        ...
        <div class="summary-row shipping-row">
            ...
            <div class="shipping-controls">
                <span class="summary-value" id="shipping_display">{!! currency_format($purchase->shipping_charge ?? 0, currency: business_currency()) !!}</span>
        ...
        <div class="shipping-input-section d-none" id="shipping-input-section">
            <div class="shipping-input-wrapper">
                <input type="number" step="0.01" min="0" id="shipping_charge_input" class="form-control shipping-input" placeholder="0.00" value="{{ $purchase->shipping_charge ?? 0 }}">
        ...
        <div class="summary-row">
            <span class="summary-label vat-label">{{ __('Vat') }} <span class="vat-rate-display">{{ $purchase->vat ? $purchase->vat->rate : 15 }}</span>%</span>
            <span class="summary-value" id="vat_display">{!! currency_format(($purchase->subTotal ?? 0) * (($purchase->vat ? $purchase->vat->rate : 15) / 100), currency: business_currency()) !!}</span>
        </div>
        <div class="summary-row summary-total">
            <span class="summary-label ">{{ __('Total') }}</span>
            <span class="summary-value" id="total_amount">{!! currency_format($purchase->totalAmount ?? 0, currency: business_currency()) !!}</span>
        </div>
    </div>
    ...
    <div class="action-buttons-section">
        @usercan('purchases.update')
            <button type="button" class="pay-bill-btn" id="open-payment-modal">{{ __('Update Payment') }}</button>
        @endusercan
    ...
    <div style="display: none;">
        <input type="hidden" name="receive_amount" id="receive_amount" value="{{ ($purchase->paidAmount ?? 0) + ($purchase->change_amount ?? 0) }}">
        <input type="hidden" id="change_amount" value="{{ $purchase->change_amount ?? 0 }}">
        <input type="hidden" id="due_amount" value="{{ $purchase->dueAmount ?? 0 }}">
        ...
        <select name="vat_id" class="vat_select">
            ...
            @foreach ($vats as $vat)
                <option value="{{ $vat->id }}" data-rate="{{ $vat->rate }}" @selected($purchase->vat_id == $vat->id)>
                    {{ $vat->name }} ({{ $vat->rate }}%)
                </option>
            @endforeach
        </select>
        <input type="hidden" name="vat_amount" id="vat_amount" value="{{ ($purchase->vat_amount ?? 0) != 0 ? $purchase->vat_amount : (($purchase->vat_percent ?? 0) != 0 ? $purchase->vat_percent : 0) }}">
        <select name="discount_type" class="discount_type">
            <option value="flat" @selected(($purchase->discount_type ?? 'flat') == 'flat')>{{ __('Flat') }}</option>
            <option value="percent" @selected(($purchase->discount_type ?? 'flat') == 'percent')>{{ __('Percent') }}</option>
        </select>
        <input type="hidden" name="discountAmount" id="discount_amount" value="{{ ($purchase->discount_type ?? 'flat') == 'percent' ? ($purchase->discount_percent ?? 0) : ($purchase->discountAmount ?? 0) }}">
        <input type="hidden" name="shipping_charge" id="shipping_charge" value="{{ $purchase->shipping_charge ?? 0 }}">
    ...
    <div class="payment-modal">
        <div class="payment-modal-header">
            ...
            <div class="payment-modal-order-info">
                <div class="payment-order-number">{{ __('Purchase') }} #<span id="modal-order-number">{{ $purchase->invoiceNumber }}</span></div>
                <div class="payment-order-total" id="modal-order-total">{!! currency_format($purchase->totalAmount ?? 0, currency: business_currency()) !!}</div>
            </div>
        </div>
        ...
        <div class="payment-amounts">
            <div class="payment-amount-field">
                ...
                <input type="text" id="modal-due-amount" readonly value="{{ $purchase->dueAmount ?? 0 }}">
            </div>
            <div class="payment-amount-field">
                <label>{{ __('Receive Amount') }}</label>
                <input type="text" id="modal-receive-amount" value="{{ ($purchase->paidAmount ?? 0) + ($purchase->change_amount ?? 0) }}">
            </div>
        </div>
        ...
        <div class="payment-summary">
            <div class="payment-summary-row">
                ...
                <span id="modal-total-bill">{!! currency_format($purchase->totalAmount ?? 0, currency: business_currency()) !!}</span>
            </div>
            <div class="payment-summary-row">
                <span>{{ __('Amount Paid') }}</span>
                <span id="modal-amount-paid">{!! currency_format($purchase->paidAmount ?? 0, currency: business_currency()) !!}</span>
            </div>
            <div class="payment-summary-row">
                <span>{{ __('Due Amount') }}</span>
                <span id="modal-due-summary">{!! currency_format($purchase->dueAmount ?? 0, currency: business_currency()) !!}</span>
            </div>
        </div>
