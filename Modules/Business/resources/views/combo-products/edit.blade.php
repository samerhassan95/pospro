            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="order-form-section p-16">
                        <form action="{{ route('business.combo-products.update', $combo->id) }}" method="POST" class="ajaxform_instant_reload">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="row">
                                        ...
                                        <div class="gpt-up-down-arrow position-relative">
                                            <select name="product_id" id="product_id" class="form-control table-select w-100 @error('product_id') is-invalid @enderror" required>
                                                ...
                                                @foreach($products as $product)
                                                    <option value="{{ $product->id }}" {{ (old('product_id', $combo->product_id) == $product->id) ? 'selected' : '' }}>
                                        ...
                                        <div class="gpt-up-down-arrow position-relative">
                                            <select name="stock_id" id="stock_id" class="form-control table-select w-100 @error('stock_id') is-invalid @enderror" required>
                                                ...
                                                @foreach($stocks as $stock)
                                                    <option value="{{ $stock->id }}" {{ (old('stock_id', $combo->stock_id) == $stock->id) ? 'selected' : '' }}>
                                        ...
                                        <div class="gpt-up-down-arrow position-relative">
                                            <select name="branch_id" id="branch_id" class="form-control table-select w-100">
                                                ...
                                                @foreach($branches as $branch)
                                                    <option value="{{ $branch->id }}" {{ (old('branch_id', $combo->branch_id) == $branch->id) ? 'selected' : '' }}>
                                                        {{ $branch->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-lg-6 mb-2">
                                        ...
                                        <input type="number" name="purchase_price" id="purchase_price" 
                                               ...
                                               value="{{ old('purchase_price', $combo->purchase_price) }}"
