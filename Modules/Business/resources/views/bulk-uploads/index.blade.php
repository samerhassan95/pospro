...
@section('main_content')
                        ...
                        <div class="bulk-upload-container">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="bulk-input">
                                    <label class="custom-file-label d-flex align-items-center gap-2">
                                        <input class="form-control" type="file" name="file" id="bulkFileInput" required
                                            style="display:none" onchange="updateFileName(this)">
                                        <button type="button" class="add-order-btn rounded-2 border-0"
                                            onclick="document.getElementById('bulkFileInput').click()">
                                            {{ __('Choose File') }}
                                        </button>
                                        <span id="fileNameDisplay" class="text-muted">{{ __('No file chosen') }}</span>
                                    </label>
                                </div>
                            </div>
                            <div class="d-flex flex-wrap align-items-center justify-content-between">
                                ...
                                @endusercan
                                <a href="{{ asset('assets/bulk-products-upload.xlsx') }}" download="bulk-products-upload.xlsx"
                            ...
                            <div class="responsive-table mt-4">
                                <table class="table" id="datatable">
                                    ...
                                    <tbody id="business-category-data">
                                        ...
                                        <tr>
                                            <td>13</td>
                                            <td class="text-start">{{ __('Vat Type') }} ({{ __('exclusive') }} / {{ __('inclusive') }})</td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-start">{{ __('The expiry date for the product (e.g., YYYY-MM-DD).') }}</td>
                                        </tr>
                                        <tr>
                                            <td>17</td>
                                            <td class="text-start">{{ __('Batch No') }}</td>
                                            <td class="text-start">{{ __('The batch number associated with the current stock.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>18</td>
                                            <td class="text-start">{{ __('Model') }}</td>
                                            <td class="text-start">{{ __('The specific model name of the product.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>19</td>
                                            <td class="text-start">{{ __('Manufacturing Date') }}</td>
                                            <td class="text-start">{{ __('The date when the product was produced.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>20</td>
                                            <td class="text-start">{{ __('Product Type') }} ({{ __('single') }} / {{ __('variant') }})</td>
                                            <td class="text-start">{{ __('Whether the product is a standard single item or part of a variation group.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>21</td>
                                            <td class="text-start">{{ __('Variations Text') }}</td>
                                            <td class="text-start">{{ __('For \'variant\' products, specify details like: "Color:Black|Size:M".') }}</td>
                                        </tr>
                                        <tr>
                                            <td>22</td>
                                            <td class="text-start">{{ __('Picture URL') }}</td>
                                            <td class="text-start">{{ __('A full URL to an image (e.g., https://example.com/image.jpg). The system will download it.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>23</td>
                                            <td class="text-start">{{ __('Size') }}</td>
                                            <td class="text-start">{{ __('The physical dimensions or size of the product.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>24</td>
                                            <td class="text-start">{{ __('Color') }}</td>
                                            <td class="text-start">{{ __('The color of the product.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>25</td>
                                            <td class="text-start">{{ __('Weight') }}</td>
                                            <td class="text-start">{{ __('The weight of the product (e.g., 500g, 1kg).') }}</td>
                                        </tr>
                                        <tr>
                                            <td>26</td>
                                            <td class="text-start">{{ __('Capacity') }}</td>
                                            <td class="text-start">{{ __('The capacity or volume (e.g., 256GB, 1.5L).') }}</td>
                                        </tr>
                                        <tr>
                                            <td>27</td>
                                            <td class="text-start">{{ __('Rack Name') }}</td>
                                            <td class="text-start">{{ __('The warehouse rack where the product is stored.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>28</td>
                                            <td class="text-start">{{ __('Shelf Name') }}</td>
                                            <td class="text-start">{{ __('The specific shelf number or ID within the rack.') }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
    ...
    <script>
        function updateFileName(input) {
            ...
            display.textContent = input.files.length > 0 ? input.files[0].name : '{{ __('No file chosen') }}';
        }
    </script>
    @endpush
@endsection
