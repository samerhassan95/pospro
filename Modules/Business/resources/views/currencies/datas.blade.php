<table class="table erp-table-content" id="currencies-data">
    <thead>
        <tr class="erp-table-thead-tr">
            <th>{{ __('SL') }}</th>
            <th>{{ __('Name') }}</th>
                ...
                <td>
                    <div class="d-flex align-items-center justify-content-center">
                        <div class="{{ ($user_currency && $currency->name == $user_currency->name) || (!$user_currency && $currency->is_default == 1) ? 'yes-badge' : 'no-badge'  }}">
                            {{ ($user_currency && $currency->name == $user_currency->name) || (!$user_currency && $currency->is_default == 1) ? __('Yes') : __('No') }}
                        </div>
                    </div>
                </td>
                <td class="text-center">
                    <button type="button" class="btn btn-sm btn-outline-primary" onclick="changeCurrency({{ $currency->id }})">
                        {{ __('Change') }}
                    </button>
                </td>
            </tr>
