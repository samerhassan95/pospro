        ...
        <div class="mb-4 d-flex loss-flex gap-3 loss-profit-container d-print-none">
            <div class="d-flex align-items-center justify-content-center gap-3">
                ...
                <div class="profit-card p-3 text-white bg-success">
                    <p class="stat-title">{{ __('Total Bank Balance') }}</p>
                    <p class="stat-value" id="totalBankBalance">{!! currency_format($totalBankBalance, currency: business_currency()) !!}</p>
                </div>
                <div class="profit-card p-3 text-white bg-primary">
                    <p class="stat-title">{{ __('Total Assets') }}</p>
                    <p class="stat-value" id="total_asset">{!! currency_format($total_asset, currency: business_currency()) !!}</p>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-bodys">
                <div class="table-header p-16 d-print-none">
                    <h4>{{ __('Balance Sheet Report') }}</h4>
                </div>

                <div class="table-header justify-content-center border-0 d-none d-block d-print-block text-center">
                    @include('business::print.header')
                    <h4 class="mt-2">{{ __('Balance Sheet Report') }}</h4>
                </div>

                <div class="table-top-form p-16">
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <form action="{{ route('business.balance-sheet-reports.filter') }}" method="post" class="report-filter-form" table="#balance-sheet-data">
                            @csrf
                    ...
                    <div class="table-top-btn-group d-print-none">
                        <ul>
                            <li>
                                <a href="{{ route('business.balance-sheet-reports.csv') }}">
                                    <img src="{{ asset('assets/images/logo/csv.svg') }}" alt="">
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('business.balance-sheet-reports.excel') }}">
            ...
            <div class="responsive-table m-0">
                <h5 class="p-3 bg-light">{{ __('Assets') }}</h5>
                <table class="table" id="datatable">
                    ...
                    <tbody id="balance-sheet-data">
                        @include('business::reports.balance-sheet.datas')
                    </tbody>
                </table>
            </div>
        </div>
