...
class AcnooPartyController extends Controller
{
    ...
    public function ledger(Request $request, $id, PartyLedgerService $service)
    {
        $party = Party::findOrFail($id);
        $ledger = $service->list($request, $id);
        
        return view('business::parties.ledger', compact('party', 'ledger'));
    }
}
