            ...
            <div class="card">
                <div class="card-bodys">
                    ...
                    <div class="table-header p-16">
                        <h4>{{ __('Bank Account List') }}</h4>
                        @usercan('banks.create')
                        <a type="button" href="#banks-create-modal" data-bs-toggle="modal"
                            class="add-order-btn rounded-2"><i class="fas fa-plus-circle me-1"></i>{{ __('Add Bank Account') }}</a>
                        @endusercan
                    </div>

                    <div class="table-top-form p-16-0">
                        <form action="{{ route('business.banks.filter') }}" method="post" class="filter-form" table="#banks-data">
                            @csrf
                    ...
                    <table class="table" id="datatable">
                        <thead>
                            <tr>
                                ...
                                @endusercan
                                <th>{{ __('SL') }}.</th>
                                <th class="text-start">{{ __('Bank Name') }}</th>
                                <th>{{ __('Balance') }}</th>
                                <th>{{ __('Status') }}</th>
                                <th>{{ __('Action') }}</th>
                            </tr>
